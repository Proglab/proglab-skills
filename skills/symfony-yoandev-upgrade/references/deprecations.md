...
`symfony-yoandev-quality` owns the file; this is what the deprecation-related half of it is for.
...
`symfony-yoandev-quality` applies to the PHPStan baseline — **it may shrink, never grow**.
...
Run it through the phpqa image like every other tool (`symfony-yoandev-quality` owns the
