...
expect it to produce the architecture in `symfony-yoandev-architecture`; expect it to remove the
...
reaches production and fails there, the sequence is `symfony-yoandev-deployment`'s — and it is
worth checking before you start that rolling back the application also rolls back
anything the migrations did, because that part is not free (`symfony-yoandev-doctrine`).
