...
description: >-
  ...
  is stored, how it is served back, who may read it — use `symfony-storage` instead.
---
...
| A `<script>` or a business rule in the template | a Stimulus controller (see `symfony-frontend`) | |
...
stream. `references/forms-and-twig.md` has it; **`symfony-frontend` arbitrates** whether
...
Adjacent skills: `symfony-doctrine` (entities, repositories, queries), `symfony-security`
(authentication, voters, `#[IsGranted]`), `symfony-frontend` (Stimulus, Turbo, Twig
components), `symfony-performance` (when `#[Cache]` is justified), `symfony-testing`, and
`symfony-storage` — this skill stops at the valid `UploadedFile`; where it is written,
