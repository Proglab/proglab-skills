...
production-like check from `symfony-local-dev` and look at the network panel.
...
this standard are idempotent (see `symfony-async`): a message that was retried after a
...
`messenger:failed:show`. That rule belongs to `symfony-async`, and a deploy is when it
gets tested.
