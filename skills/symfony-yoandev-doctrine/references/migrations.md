...
Split it in two, which is the expand/contract shape `symfony-yoandev-deployment` describes:
...
2. a **console command** for the data itself, batched, resumable, `--force`-guarded,
   run when someone is watching (see `symfony-yoandev-console`).
...
The full deploy sequence and where migrations sit in it belong to `symfony-yoandev-deployment`.
