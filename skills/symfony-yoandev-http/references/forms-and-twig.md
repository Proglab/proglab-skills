...
stream is arbitrated in `symfony-yoandev-frontend`**, on one question: is there state to remember
...
measurement first — `symfony-yoandev-performance` covers when it is justified and why
...
- **No inline `<script>`.** Behaviour goes in a Stimulus controller, and no business rule
  goes in JavaScript at all — see `symfony-yoandev-frontend`.
- **Twig filters and functions via `Twig\Attribute\AsTwigFilter` and friends**, never an
  `AbstractExtension` class. `symfony-yoandev-frontend` owns that rule, with the package and
...
standard are in `symfony-yoandev-frontend`.
