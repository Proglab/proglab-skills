...
in `symfony-architecture` points at when a use case ends with "and notify the author".
...
`symfony-local-dev`. **Nothing appears in Mailpit until a worker consumes the
