...
or by a FormType's `data_class`. The controller mechanics belong to `symfony-http`; what
...
because the entity is not what gets validated. Details in `symfony-http`.
...
final readonly class ShelfStatsRow
{
    public function __construct(
        ...
        // what "raw values" means. See symfony-doctrine.
...
collection per row. Writing the query is `symfony-doctrine`'s subject; the constructor
