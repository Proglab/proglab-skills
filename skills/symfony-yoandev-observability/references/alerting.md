...
The mechanism behind `symfony-yoandev-async`'s rule — "a message in the failure queue must
...
constraint `symfony-yoandev-deployment` documents for `messenger:stop-workers` applies, so with
...
`#[Target]` trap in `symfony-yoandev-architecture`; the controller below survives it.
