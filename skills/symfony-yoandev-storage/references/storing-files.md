...
mechanism is in `symfony-yoandev-architecture`, `references/dependency-injection.md`.
...
smoke test) are `symfony-yoandev-console`'s; making it periodic is `symfony-yoandev-async`'s.
...
`failOnPhpunitNotice="true"` to make that red (`symfony-yoandev-quality`).
