...
(`symfony-yoandev-observability`), and `#[Autowire(service: '…')]` typed to an interface the
...
- **`#[AutowireLocator]` again when the fan-out must tolerate a broken member.** The
  ...
  `symfony-yoandev-observability` has the worked example.
...
`symfony-yoandev-performance` before adding one.
