...
(`symfony-yoandev-architecture`), so it must be able to type-hint `EntityManagerInterface`: the
...
| Letting `Controller` depend on `Repository` "it is only a find()" | Go through the service, which takes the id and owns the 404. The EntityValueResolver is for the one case where a voter needs the subject first (`symfony-yoandev-architecture`) |
