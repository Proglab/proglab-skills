---
name: symfony-yoandev-deployment
description: >-
...
do *before* shipping — see `symfony-yoandev-local-dev`. For the CI pipeline that must pass
first, see `symfony-yoandev-quality`. This skill starts once the image is about to be built, and
stops at *where* logs and metrics go: **what the application emits — channels, log
levels, redaction, alerting, health checks — is `symfony-yoandev-observability`'s.** This skill
...
belongs to `symfony-yoandev-doctrine`. This skill only says when it runs.
...
| Symptom | Cause |
|---|---|
| Blank page, 500, nothing in the log | `APP_DEBUG=0` hides the error but the log handler is misconfigured — check logs go to stderr. If the handler is right and the log is still silent, that is `fingers_crossed` doing its job, and the question belongs to `symfony-yoandev-observability` |
