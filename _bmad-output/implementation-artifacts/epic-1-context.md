...
- **Connexion** : refus d'un compte désactivé sans révéler la validité du mot de passe ; message générique unique sur identifiants faux ; ralentissement progressif des échecs répétés affichant les secondes restantes ; aucun blocage définitif. Réinitialisation par lien email à usage unique et expirable, avec réponse identique que le compte existe ou non.
- **Trois langues** : tout texte d'interface, message d'erreur et email du socle en français, anglais et néerlandais ; le français est la langue source. Les langues supportées sont en code, les langues **actives** en base de données — jamais en variable d'environnement. La langue du compte est mémorisée et appliquée à ses emails.
...
- **Sécurité** : mots de passe hachés selon l'état de l'art ; CSRF obligatoire sur toute écriture, sans exception ; aucune donnée sensible dans les journaux applicatifs.
...
- **Couture assumée** : le critère « le premier Super admin voit la roadmap » ne sera pleinement vérifiable qu'à l'Epic 3. L'Epic 1 livre la connexion et une page d'accueil ; l'Epic 3 en fait la roadmap. La fonction est complète, seul le critère se termine plus tard.
- **Audit** : aucune écriture d'audit n'est câblée dans cet epic. L'Epic 4 raccordera rétroactivement les actions livrées ici.
...
- **Emails** : tout email passe par Messenger sur transport Doctrine, avec retry et file d'échec. Un message porte des scalaires et une locale explicite, jamais une entité ni une dépendance au contexte de requête, et n'est consommable qu'après le commit. Worker livré en service systemd piloté par le déploiement.
...
- **Limitation de débit** : `login_throttling` natif adossé au rate-limiter, avec des limiteurs nommés distincts pour la demande de réinitialisation. Les secondes restantes sont lues sur le limiteur, jamais déduites d'un message.
- **URL** : chemins en anglais et invariables, sans préfixe de langue ; le `?lang=` de la page de connexion est la seule exception. Les noms de route sont préfixés `app_` ; les chemins concernés par cet epic sont `/login`, `/password/forgot`, `/password/reset/{token}` et `/` pour l'accueil.
...
- **Interaction** : clavier d'abord, aucun raccourci global inventé ; Échap ferme l'élément flottant le plus haut et rend le focus à son déclencheur ; `:focus-visible` sur tout élément interactif ; le survol n'ajoute qu'une teinte, jamais une action ; une seule couche flottante à la fois ; ni glisser-déposer, ni clic droit, ni double clic, ni geste de balayage.
- **Responsive** : quatre paliers jusqu'à 320 px à zoom 400 % sans défilement horizontal du body — seuls les tableaux et les blocs à chasse fixe défilent dans leur conteneur. Mouvement limité à quatre transitions courtes, toutes ramenées à zéro sous `prefers-reduced-motion`.
...
- **Pages d'erreur** : 403, 404 et 500 partagent un gabarit court, un message traduit et un lien de retour à l'accueil, sans aucun détail technique — ni code HTTP, ni trace, ni nom de classe. Rendues par le template d'erreur du framework, donc aussi hors Turbo.
- **Textes** : la version française de référence de chaque moment vient du tableau de ton du contrat UX — vouvoiement, phrases courtes, jamais d'identifiant technique, un message d'erreur dit toujours quoi faire ensuite. Ces chaînes sont la source dont l'anglais et le néerlandais sont les traductions.
...
- La 1.6 livre les entités Utilisateur et Rôle ; les trois rôles nommés existent ici, mais leurs permissions n'arrivent qu'à l'Epic 2.
...
- La 1.12 documente ce que les onze précédentes ont posé, et fixe la convention d'en-tête YAML des stories que l'Epic 3 consommera.
