...
- **Dérivabilité** : rien ne suppose un client particulier ; un dérivé n'édite aucun fichier de `src/Core/` ni de `config/`. Toute configuration propre à un dérivé vit dans des variables d'environnement ou dans des fichiers prévus pour être remplacés. Le rebranding se limite à trois choses : les variables de couleur du thème, les fichiers de marque, et le nom du dérivé en configuration.
...
- **Qualité** : test écrit d'abord et vu rouge ; chaque exigence fonctionnelle couverte par au moins un test fonctionnel ; base de données de test identique à celle de production, jamais SQLite. La porte de qualité compte six catégories et une story ne lui ajoute ni cible ni job.
...
- **Parité des deux racines dans le conteneur** : le glob des modules et son miroir de test excluent exactement la même liste de dossiers que celui du socle — au moins `Enum`, `Exception` et `Message` en plus de `Dto` et `Entity` — et chaque exclusion est justifiée sur place. Un module ne doit jamais proposer comme service ce que le socle exclut.
- **Frontière mécanisée** : deptrac vérifie quatre règles bloquantes en CI — couches proglab dans chaque racine, `Core → Module` interdit, `Module → Module` interdit, `Module → Core` interdit hors `Core\Contract`. Le filet `UndeclaredModule` ne protège pas contre l'oubli inverse : sortir un module du `must_not` sans déclarer son bloc de couche, sa ligne de ruleset et son nom de racine le laisse sans couche, donc libre d'atteindre `Core`. Un test doit nommer l'édition manquante, y compris pour les modules d'un dérivé, et les commentaires de configuration doivent décrire ce que le filet tient réellement.
...
- **Frontend** : AssetMapper et Tailwind 4 via le bundle Symfony, sans bundler ni étape Node. Kit shadcn copié dans le dépôt (un seul kit, une seule bibliothèque d'icônes). Aucun Live Component dans le socle. Le gabarit appelant `asset()` en production, le composant d'asset est une dépendance de production et non de développement.
- **Déploiement** : Deployer en SSH sur `releases/` · `current/` · `shared/`, Apache et PHP-FPM, MySQL et SMTP du serveur client. Les migrations s'exécutent à chaque release, le worker est redémarré par le déploiement, et la date de la release servira de « date du dernier déploiement » à l'Epic 3. La sauvegarde est une responsabilité nommée du serveur client, documentée avec sa procédure de restauration et l'exigence qu'elle ait été exécutée une fois avant la mise en service.
...
- **Surface de marque** : le nom du dérivé vient de sa configuration et alimente à la fois le `<title>` et le `h1` — aucun nom de socle en dur, jamais deux noms différents sur une page. L'icône du navigateur est déclarée dans le gabarit et servie depuis un répertoire de marque remplaçable sans éditer `src/Core/` ni `config/`. Le contrat de rebranding est énoncé de façon identique dans la feuille de marque, le README et le guide de dérivation, et un test de gabarit garde l'icône et le `h1` au même titre que le titre. Le logo de la sidebar, troisième pièce du contrat, arrive avec la coque de l'application à l'Epic 2.
...
- La 1.12 documente ce que les autres stories ont posé, et fixe la convention d'en-tête YAML des stories que l'Epic 3 consommera. Elle nomme aussi les limites assumées du socle, dont l'envoi déjà en file qui part à l'ancienne adresse après une anonymisation (Epic 4).
...
- La 1.14 referme le reste de marque reporté par les 1.4, 1.5 et 1.11, et retire du guide de la 1.12 la limite « la surface de marque n'est pas encore complète ». Elle laisse le logo de la sidebar à la coque de l'Epic 2.
- La 1.15 corrige deux défauts de la 1.1 relevés en écrivant le guide (1.12) : le glob de découverte des services d'un module et le filet deptrac de la frontière. Elle touche donc `config/` et la configuration deptrac, que la 1.12 s'interdisait, et efface du guide les deux limites correspondantes. Ses deux volets restent séparables en deux lots si l'implémentation le préfère.
